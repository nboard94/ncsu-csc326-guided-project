	...
	public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
		...
		try {
			if ( form.validateChange( user ) ) {
				...
				String body = "Hello " + firstName + ", \n\nYou have successfully updated your password for iTrust2.\n";
				body += "\nIf you did not request a password reset, please contact a system administrator.\n\n--iTrust2 Admin";

				try {
					EmailUtil.sendEmail( addr, "iTrust2 Password Update Success", body );
					LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
							"Successfully updated password for user " + user.getUsername() );
				}
				catch ( final MessagingException e ) {
					LoggerUtil.log( TransactionType.EMAIL_NOT_SENT, user.getUsername(),
							"Password update email could not be sent for user " + user.getUsername() );
				}
				return new ResponseEntity( successResponse( "Password updated successfully" ), HttpStatus.OK );
			}
