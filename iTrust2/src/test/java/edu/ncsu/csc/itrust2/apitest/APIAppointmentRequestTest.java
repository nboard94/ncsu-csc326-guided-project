...
public class APIAppointmentRequestTest {
	...
	/**
	 * Tests AppointmentControllerHCp
	 *
	 * @throws Exception
	 */
	@Test
	public void testAppControllerHCp () throws Exception {
		...
		form.setAppointment(appointmentForm.getId());
		form.setAction("accept");
		
		assertEquals("accept", form.getAction());
		assertEquals("123", form.getAppointment());
	//	mvc.perform(get("api/v1/hcp/viewAppointmentRequests").contentType( MediaType.APPLICATION_JSON )
			//	.content( TestUtils.asJsonString( form ) ) ).andExpect(status().isAccepted());
